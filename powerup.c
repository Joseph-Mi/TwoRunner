...
void draw_blue_block_with_branches() {
    ...
    // Pure blue color
    short int yellow_color = 0xFFE0;
    
    // Draw the 2x2 center block
    for (int x = center_x - 1; x < center_x + 1; x++) {
        for (int y = center_y - 1; y < center_y + 1; y++) {
            plot_pixel(x, y, yellow_color);
        }
    }
    
    plot_pixel(center_x - 2, center_y - 2, yellow_color);
    plot_pixel(center_x + 1, center_y - 2, yellow_color);
    plot_pixel(center_x - 2, center_y + 1, yellow_color);
    plot_pixel(center_x + 1, center_y + 1, yellow_color);
}
